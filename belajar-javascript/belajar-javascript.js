function belajarArray() {
  ...
  //manambahkan array itu ganteng banget ke dalam array pada index terakhir
...
function belajarObject() {
  ...
  //memanggil semua nama di dalam array object (biodata2)
...
function testLoopingFor() {
  const nama = ['daffa', 'itu', 'sangat', 'ganteng'];
  for (let i = 0; i < nama.length; i++) {
    console.log(nama[i]);
  }
}

function testLoopingDoWhile() {
  const nama = ['daffa', 'itu', 'sangat', 'ganteng', 'sih'];
  let i = 0;
  while (i < nama.length) {
    console.log(nama[i]);
    i++;
